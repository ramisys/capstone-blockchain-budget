...
/**
 * Confirmation dialog used before archiving (soft-deleting) a document. The
 * version history and blockchain anchors are preserved.
 */
const DocumentArchiveDialog: React.FC<DocumentArchiveDialogProps> = ({
  isOpen,
  onClose,
  onConfirm,
  isLoading = false,
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-7 py-6 space-y-5">
          <div className="p-5 bg-red-50/80 border border-red-200/80 rounded-2xl flex items-start gap-3">
            ...
            <div className="text-sm text-red-800 space-y-1">
              <p className="font-bold">Are you sure you want to archive this document?</p>
              {documentCode && (
                <p>
                  Document <strong>&quot;{documentCode}&quot;</strong> will be removed from the
                  active list. All versions and their blockchain anchors are preserved for audit.
                </p>
              )}
            </div>
          </div>
          <p className="text-sm text-slate-500">
            Archived documents can still be viewed and verified, but can no longer be replaced or
            have their metadata edited.
          </p>
        </div>
...
export { DocumentArchiveDialog };
export default DocumentArchiveDialog;
