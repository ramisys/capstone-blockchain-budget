...
interface BudgetProgramDeleteDialogProps {
  isOpen: boolean;
  onClose: () => void;
  programId: string;
  programName: string;
...
const BudgetProgramDeleteDialog: React.FC<BudgetProgramDeleteDialogProps> = ({
  isOpen,
  onClose,
  programId,
  programName,
  ...
  const handleDelete = async () => {
    try {
      await onSubmit(programId);
      showToast('Budget program deleted successfully', 'success');
      onClose();
    } catch (error: any) {
      showToast(
        error.response?.data?.message || 'Failed to delete budget program',
        'error'
      );
    }
  };
        ...
        <div className="px-7 py-5 border-b border-slate-100 bg-white flex items-center justify-between shrink-0">
          <div>
            ...
            <DialogDescription className="text-sm text-slate-500 mt-1">
              Permanently remove this budget program.
            </DialogDescription>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-7 py-6 space-y-5">
          <div className="p-5 bg-red-50/80 border border-red-200/80 rounded-2xl flex items-start gap-3">
            ...
            <div className="text-sm text-red-800 space-y-1">
              ...
              <p>
                Are you sure you want to delete the budget program <strong>"{programName}"</strong>? This action cannot be undone.
              </p>
            </div>
          </div>
          <p className="text-sm text-slate-500">
            This will permanently remove the budget program and all associated data.
          </p>
        </div>
...
export { BudgetProgramDeleteDialog };
export default BudgetProgramDeleteDialog;
