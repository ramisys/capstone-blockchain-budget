...
const BudgetProgramCreateDialog: React.FC<BudgetProgramCreateDialogProps> = ({
  isOpen,
  onClose,
  onSubmit,
  isLoading = false,
  ...
  const onFormSubmit = async (data: BudgetProgramFormData) => {
    try {
      await onSubmit(data);
      reset();
      showToast('Budget program created successfully', 'success');
      onClose();
    } catch (error: any) {
      showToast(
        error.response?.data?.message || 'Failed to create budget program',
        'error'
      );
    }
  };
        ...
        <div className="px-7 py-5 border-b border-slate-100 bg-white flex items-center justify-between shrink-0">
          <div>
            ...
            <DialogDescription className="text-sm text-slate-500 mt-1">
              Create a new budget program for financial planning and tracking.
            </DialogDescription>
          </div>
          <button
            type="button"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(val: any) => setValue('departmentId', val, { shouldValidate: true })}
                    disabled={isFormDisabled}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(val: any) => setValue('budgetCategoryId', val, { shouldValidate: true })}
                    disabled={isFormDisabled}
                  >
                    <SelectTrigger>
...
export { BudgetProgramCreateDialog };
export default BudgetProgramCreateDialog;
