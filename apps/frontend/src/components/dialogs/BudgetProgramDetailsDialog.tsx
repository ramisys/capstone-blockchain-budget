...
interface BudgetProgramDetailsDialogProps {
  isOpen: boolean;
  onClose: () => void;
  programId: string;
  initialData: any; // Ideally typed as BudgetProgram
}

const BudgetProgramDetailsDialog: React.FC<BudgetProgramDetailsDialogProps> = ({
  isOpen,
  onClose,
  programId,
  initialData
        ...
        <div className="px-7 py-5 border-b border-slate-100 bg-white flex items-center justify-between shrink-0">
          <div>
            ...
            <DialogDescription className="text-sm text-slate-500 mt-1">
              View budget program information.
            </DialogDescription>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-7 py-6 space-y-7">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="space-y-1">
                <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Budget Category</p>
                <p className="text-sm font-semibold text-slate-900">
                  {initialData.budgetCategory?.name || 'Not assigned'}
                </p>
              </div>
            </div>
          </div>
...
export { BudgetProgramDetailsDialog };
export default BudgetProgramDetailsDialog;
