...
interface FiscalYearActivateDialogProps {
  isOpen: boolean;
  onClose: () => void;
  fiscalYearId: string;
...
const FiscalYearActivateDialog: React.FC<FiscalYearActivateDialogProps> = ({
  isOpen,
  onClose,
  fiscalYearId,
  fiscalYearName,
  ...
  const handleActivate = async () => {
    try {
      await onSubmit(fiscalYearId);
      showToast('Fiscal year set as active successfully', 'success');
      onClose();
    } catch (error: any) {
      showToast(
        error.response?.data?.message || 'Failed to set fiscal year as active',
        'error'
      );
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-lg! p-0! gap-0! rounded-2xl shadow-2xl border border-slate-200/90 bg-white overflow-hidden max-h-[90vh] flex! flex-col">
        ...
        <div className="px-7 py-5 border-b border-slate-100 bg-white flex items-center justify-between shrink-0">
          <div>
            ...
            <DialogDescription className="text-sm text-slate-500 mt-1">
              Activate a fiscal year for budget planning and allocation.
            </DialogDescription>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-7 py-6 space-y-5">
          <div className="p-5 bg-emerald-50/80 border border-emerald-200/80 rounded-2xl flex items-start gap-3">
            <Info className="w-5 h-5 text-emerald-600 shrink-0 mt-0.5" />
            <div className="text-sm text-emerald-800 space-y-1">
              <p className="font-bold">Confirm Activation</p>
              <p>
                Are you sure you want to set <strong>"{fiscalYearName}"</strong> as the active fiscal year? This will deactivate all other fiscal years.
              </p>
            </div>
          </div>
          <p className="text-sm text-slate-500">
            Setting this fiscal year as active will automatically deactivate all other fiscal years.
          </p>
        </div>
...
export { FiscalYearActivateDialog };
export default FiscalYearActivateDialog;
