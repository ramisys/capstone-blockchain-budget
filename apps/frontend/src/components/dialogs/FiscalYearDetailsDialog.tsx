...
interface FiscalYearDetailsDialogProps {
  isOpen: boolean;
  onClose: () => void;
  fiscalYearId: string;
  initialData: any; // Would ideally be typed as FiscalYear
}

const FiscalYearDetailsDialog: React.FC<FiscalYearDetailsDialogProps> = ({
  isOpen,
  onClose,
  fiscalYearId,
  initialData
            ...
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div className="space-y-0.5">
                ...
                <h3 className="text-lg font-bold text-slate-900">
                  {initialData.isActive ? 'Active Operating Year' : 'Fiscal Year'}
                </h3>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Badge variant={initialData.status === 'Active' ? 'default' : initialData.status === 'Inactive' ? 'secondary' : 'destructive'}>
                  {initialData.status}
                </Badge>
              </div>
            </div>
...
export { FiscalYearDetailsDialog };
export default FiscalYearDetailsDialog;
