...
interface BudgetCategoryDeleteDialogProps {
  isOpen: boolean;
  onClose: () => void;
  categoryId: string;
  categoryName: string;
...
const BudgetCategoryDeleteDialog: React.FC<BudgetCategoryDeleteDialogProps> = ({
  isOpen,
  onClose,
  categoryId,
  categoryName,
  ...
  const handleDelete = async () => {
    try {
      ...
      showToast(
        error.response?.data?.message || 'Failed to delete budget category',
        'error'
      );
    }
  };
        ...
        <div className="px-7 py-5 border-b border-slate-100 bg-white flex items-center justify-between shrink-0">
          <div>
            ...
            <DialogDescription className="text-sm text-slate-500 mt-1">
              Permanently remove this budget category.
            </DialogDescription>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-7 py-6 space-y-5">
          <div className="p-5 bg-red-50/80 border border-red-200/80 rounded-2xl flex items-start gap-3">
            ...
            <div className="text-sm text-red-800 space-y-1">
              ...
              <p>
                Are you sure you want to delete the budget category <strong>"{categoryName}"</strong>? This action cannot be undone.
              </p>
            </div>
          </div>
          <p className="text-sm text-slate-500">
            This will permanently remove the budget category and all associated data.
          </p>
        </div>
...
export { BudgetCategoryDeleteDialog };
export default BudgetCategoryDeleteDialog;
