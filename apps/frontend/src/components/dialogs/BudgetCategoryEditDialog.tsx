...
interface BudgetCategoryEditDialogProps {
  isOpen: boolean;
  onClose: () => void;
  categoryId: string;
  initialData: any;
...
const BudgetCategoryEditDialog: React.FC<BudgetCategoryEditDialogProps> = ({
  isOpen,
  onClose,
  categoryId,
  initialData,
  ...
  const formMethods = useForm<BudgetCategoryFormData>({
  ...
  const onFormSubmit = async (data: BudgetCategoryFormData) => {
    try {
      await onSubmit({ id: categoryId, data });
      showToast('Budget category updated successfully', 'success');
      onClose();
    } catch (error: any) {
      showToast(
        error.response?.data?.message || 'Failed to update budget category',
        'error'
      );
    }
  };
...
export { BudgetCategoryEditDialog };
export default BudgetCategoryEditDialog;
