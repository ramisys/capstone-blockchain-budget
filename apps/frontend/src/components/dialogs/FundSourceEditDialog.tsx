...
interface FundSourceEditDialogProps {
  isOpen: boolean;
  onClose: () => void;
  fundSourceId: string;
  initialData: any;
...
const FundSourceEditDialog: React.FC<FundSourceEditDialogProps> = ({
  isOpen,
  onClose,
  fundSourceId,
  initialData,
  ...
  const formMethods = useForm<FundSourceFormData>({
  ...
  const onFormSubmit = async (data: FundSourceFormData) => {
    try {
      await onSubmit({ id: fundSourceId, data });
      showToast('Fund source updated successfully', 'success');
      onClose();
    } catch (error: any) {
      showToast(
        ...
        <div className="px-7 py-5 border-b border-slate-100 bg-white flex items-center justify-between shrink-0">
          <div>
            ...
            <DialogDescription className="text-sm text-slate-500 mt-1">
              Update the details of this fund source.
            </DialogDescription>
          </div>
          <button
            type="button"
...
export { FundSourceEditDialog };
export default FundSourceEditDialog;
