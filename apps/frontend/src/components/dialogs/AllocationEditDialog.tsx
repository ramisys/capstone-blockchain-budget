...
import { useUpdateAllocation } from '../../hooks/useAllocations';
...
interface AllocationEditDialogProps {
  isOpen: boolean;
  onClose: () => void;
  allocation?: Allocation | null;
}

const AllocationEditDialog: React.FC<AllocationEditDialogProps> = ({
  isOpen,
  onClose,
  allocation,
}) => {
  ...
  const { mutateAsync: updateAllocation, isPending: isUpdating } = useUpdateAllocation();
  ...
  const isFormDisabled = isSubmitting || isUpdating;
  ...
  const onFormSubmit = async (data: AllocationFormData) => {
    ...
    try {
      ...
      await updateAllocation({ id: allocation.id, data: payload });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-3xl! p-0! gap-0! rounded-2xl shadow-2xl border border-slate-200/90 bg-white overflow-hidden max-h-[90vh] flex! flex-col">
        ...
        <div className="px-7 py-5 border-b border-slate-100 bg-white flex items-center justify-between shrink-0">
          <div>
            ...
            <DialogDescription className="text-sm text-slate-500 mt-1">
              Update the allocation details. Changes apply only while the allocation is in Draft status.
            </DialogDescription>
          </div>
          <button
            type="button"
        ...
        {allocation && (
          <div className="px-7 py-3 bg-slate-50/70 border-b border-slate-100 flex flex-wrap items-center justify-between gap-2 shrink-0">
            <span className="text-sm text-slate-500">Editing allocation</span>
            <span className="font-mono text-sm font-bold text-indigo-700">{allocation.allocationCode}</span>
          </div>
        )}
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-7 py-6 space-y-7">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
                <div className="space-y-2">
                  <Label htmlFor="fiscalYearId" className="text-sm font-semibold text-slate-800">
                  ...
                  <input
                    ...
                    type="text"
                    disabled
                    value={allocation?.fiscalYear?.code ?? '—'}
                    className={`${inputClassName(false)} bg-slate-50 opacity-60 cursor-not-allowed`}
                  />
                </div>
                <FieldSelect
                  id="departmentId"
...
export { AllocationEditDialog };
export default AllocationEditDialog;
