...
const FiscalYearDeleteDialog: React.FC<FiscalYearDeleteDialogProps> = ({
  isOpen,
  onClose,
  fiscalYearId,
  fiscalYearName,
  ...
  const handleDelete = async () => {
    try {
      ...
      showToast('Fiscal year deleted successfully', 'success');
      onClose();
    } catch (error: any) {
      showToast(
        ...
        <div className="px-7 py-5 border-b border-slate-100 bg-white flex items-center justify-between shrink-0">
          <div>
            ...
            <DialogDescription className="text-sm text-slate-500 mt-1">
              Permanently remove this fiscal year.
            </DialogDescription>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-7 py-6 space-y-5">
          <div className="p-5 bg-red-50/80 border border-red-200/80 rounded-2xl flex items-start gap-3">
            ...
            <div className="text-sm text-red-800 space-y-1">
              ...
              <p>
                Are you sure you want to delete the fiscal year <strong>"{fiscalYearName}"</strong>? This action cannot be undone.
              </p>
            </div>
          </div>
          <p className="text-sm text-slate-500">
            This will permanently remove the fiscal year and all associated data.
          </p>
        </div>
